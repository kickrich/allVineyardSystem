import { apiGet, apiPost, apiPatch, apiDelete, apiPostForm, apiPatchForm, clearApiSession, apiRequest } from './client';
...
export async function fetchLocalVideosCatalog(rowsCount) {
  if (!Number.isFinite(rowsCount) || rowsCount < 1) {
    throw new Error('rowsCount must be >= 1');
  }
  const response = await apiGet(
    `/api/v1/local_videos?rows_count=${encodeURIComponent(rowsCount)}`
  );
  const data = extractData(response);
  return Array.isArray(data) ? data : [];
}

export async function downloadLocalVideoBlob(downloadPath) {
  if (!downloadPath) throw new Error('downloadPath is required');
  const path = downloadPath.startsWith('/') ? downloadPath : `/${downloadPath}`;
  const response = await apiRequest(path, { timeoutMs: 600000 });
  if (!response.ok) {
