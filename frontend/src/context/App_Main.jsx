...
const AI_PENDING_SESSION_KEY = 'ai_pending_by_mission_id';

function readAiPendingFromSession() {
  if (typeof window === 'undefined') return {};
  try {
    const raw = sessionStorage.getItem(AI_PENDING_SESSION_KEY);
    if (!raw) return {};
...
function writeAiPendingToSession(pending) {
  if (typeof window === 'undefined') return;
  try {
    if (!pending || typeof pending !== 'object' || !Object.keys(pending).length) {
      sessionStorage.removeItem(AI_PENDING_SESSION_KEY);
      return;
    }
    sessionStorage.setItem(AI_PENDING_SESSION_KEY, JSON.stringify(pending));
  } catch {
    // ignore storage quota / serialization issues
  }
}

function buildAiPendingStatusMessage(statusPayload) {
  if (!statusPayload || statusPayload.available === false) {
    return 'Ожидание отправки видео в Vineyard';
  }
  const progressNum = Number(statusPayload.processing_progress ?? statusPayload.processingProgress);
  const videoStatus = statusPayload.video_status ?? statusPayload.videoStatus ?? null;
  if (videoStatus === 'completed' && (!Number.isFinite(progressNum) || progressNum >= 100)) {
    return 'Синхронизация результатов…';
  }
  if (videoStatus === 'error') {
    return 'Ошибка обработки в Vineyard';
  }
  return null;
}

function buildAiPendingEntryFromStatus(statusPayload, { missionStatus = null } = {}) {
  const normalizedStatus = String(missionStatus ?? '').toLowerCase();
  ...
  if (normalizedStatus === 'in_progress' || normalizedStatus === 'planned' || normalizedStatus === 'approved') {
    phase = 'mission';
  } else if (!available) {
    phase = 'waiting';
  }
  ...
  return {
    phase,
    videoStatus,
    missionStatus: normalizedStatus || null,
    processingProgress: Number.isFinite(progressNum) ? Math.min(100, Math.max(0, progressNum)) : null,
    shardsCount: shardsCount ?? null,
    processedShards: processedShards ?? null,
    rowsCount: rowsCount ?? null,
    statusMessage: buildAiPendingStatusMessage(statusPayload),
    updatedAt: Date.now(),
  };
}

function shouldDropAbandonedCompletedPending(pendingObj, statusPayload, isUploadInProgress = false) {
  if (isUploadInProgress) return false;
  if (pendingObj?.phase === 'mission') return false;
  const missionStatus = String(pendingObj?.missionStatus ?? '').toLowerCase();
  if (missionStatus !== 'completed') return false;
  return statusPayload?.available === false;
}

function shouldRestorePendingWithoutAiResult(mission) {
...
function isMissionNotFoundError(err) {
  const msg = String(err?.message ?? err ?? '').toLowerCase();
  return (
    msg.includes('404') ||
    msg.includes('not found') ||
    msg.includes("couldn't find mission") ||
...
export function AppProvider({ children }) {
  ...
  const applyTemplateToDrone = useCallback((droneId, tplId) => {
    ...
    setDrones((prev) => {
      ...
      if (drone && path.length >= 2) {
        ...
        return next.map((d) =>
          d.id === droneId
            ? { ...d, path, missionParameters: params, readyForMissionStart: false }
            : d
        );
      }
      return next;
  ...
  const [aiPendingByMissionId, setAiPendingByMissionId] = useState(() => readAiPendingFromSession());
  const aiPendingByMissionIdRef = useRef(aiPendingByMissionId);
  useEffect(() => {
    aiPendingByMissionIdRef.current = aiPendingByMissionId;
    writeAiPendingToSession(aiPendingByMissionId);
    Object.keys(aiPendingByMissionId).forEach((missionKey) => {
      const missionId = Number(missionKey);
      if (Number.isFinite(missionId)) trackMissionId(missionId);
    });
  ...
  const dropStalePendingMission = useCallback((missionId) => {
    const numericId = Number(missionId);
    if (!Number.isFinite(numericId)) return;
    const missionKey = String(numericId);
    untrackMissionId(numericId);
    missionDroneByMissionIdRef.current.delete(numericId);
    ...
    const bootstrapBackend = async () => {
      ...
      try {
        ...
        try {
          ...
          if (!cancelled && Array.isArray(missions)) {
            const existingMissionIds = new Set(
              missions.map((m) => Number(m?.id)).filter((id) => Number.isFinite(id))
            );
            setAiPendingByMissionId((prev) => {
              let changed = false;
              const copy = { ...prev };
              Object.keys(copy).forEach((key) => {
                ...
                if (!Number.isFinite(missionId) || existingMissionIds.has(missionId)) return;
                delete copy[key];
            ...
            if (!cancelled && Object.keys(restoredResults).length > 0) {
              ...
              setAiPendingByMissionId((prev) => {
                let changed = false;
                const copy = { ...prev };
                Object.keys(restoredResults).forEach((missionKey) => {
                  if (!(missionKey in copy)) return;
                  delete copy[missionKey];
                  untrackMissionId(Number(missionKey));
                  changed = true;
                });
                return changed ? copy : prev;
              });
            ...
            for (const mission of recentMissions) {
              ...
              const missionId = Number(mission?.id);
              if (!Number.isFinite(missionId)) continue;
              if (restoredResults[String(missionId)]) continue;
              if (!shouldRestorePendingWithoutAiResult(mission)) continue;

              const missionStatus = String(mission?.status ?? '').toLowerCase();
              try {
                const statusPayload = await fetchMissionProcessingStatusFromBackend(missionId);
                const entry = buildAiPendingEntryFromStatus(statusPayload, { missionStatus });
                if (shouldDropAbandonedCompletedPending(entry, statusPayload, false)) {
                  continue;
                }
                restoredPending[String(missionId)] = entry;
              } catch {
                const fallbackEntry = {
                  phase: missionStatus === 'completed' ? 'processing' : 'mission',
                  missionStatus: missionStatus || null,
                  updatedAt: Date.now(),
                };
                if (shouldDropAbandonedCompletedPending(fallbackEntry, { available: false }, false)) {
                  continue;
                }
                restoredPending[String(missionId)] = fallbackEntry;
              }
            ...
            if (!cancelled) {
              setAiPendingByMissionId((prev) => {
                ...
                Object.keys(copy).forEach((key) => {
                  const missionId = Number(key);
                  if (!Number.isFinite(missionId)) return;
                  if (restoredResults[key] || restoredPending[key]) return;
                  const mission = recentMissions.find((m) => Number(m?.id) === missionId);
                  if (!mission) return;
                  const missionStatus = String(mission?.status ?? '').toLowerCase();
                  if (missionStatus !== 'completed') return;
                  const pendingObj = copy[key] === true ? {} : (copy[key] && typeof copy[key] === 'object' ? copy[key] : {});
                  if (shouldDropAbandonedCompletedPending(
                    { ...pendingObj, missionStatus },
                    { available: false },
                    false
                  )) {
                    delete copy[key];
            ...
            if (!cancelled && Object.keys(restoredPending).length > 0) {
              setAiPendingByMissionId((prev) => ({ ...prev, ...restoredPending }));
            }
            }
          }
        } catch (e) {
  ...
  const uploadSingleVideoBlobViaMultipart = async ({
    missionId,
    droneId,
    blob,
    filename = null,
    ...
    const resolvedContentType = contentType || VIDEO_BACKEND_CONTENT_TYPE;
    const resolvedFilename =
      filename ||
      `mission_${missionId}_drone_${droneId}${filenameRowSuffix}_${Date.now()}${
        resolvedContentType === 'video/mp4' ? '.mp4' : '.webm'
      }`;
    ...
    const uploadMode = init?.upload_mode || 'multipart';
    if (!uploadSessionId) {
      throw new Error('multipart_init returned unexpected data');
    }

    if (uploadMode === 'simple_put') {
      const putUrl = init?.put_url;
      if (!putUrl) {
        throw new Error('simple_put init missing put_url');
      }
      const headers = init?.put_headers ?? {};
      ...
      if (!putRes.ok) {
        throw new Error(`PUT object failed with ${putRes.status}`);
      }
    ...
    if (!Number.isFinite(totalParts) || !Number.isFinite(chunkSizeBytes)) {
      throw new Error('multipart_init returned unexpected data');
    }
  ...
  const autoUploadLocalVideosViaMultipartForMission = async (droneId, missionId) => {
    if (missionId == null) return;
    ...
    const rowsCount =
      (Number.isInteger(rowSplitState?.rowsCount) && rowSplitState.rowsCount > 0
    ...
    try {
      ...
      if (!catalog.length) {
        throw new Error(`В LOCAL_VIDEOS_FOLDER нет видео для rows_count=${rowsCount}`);
      }

      for (const fileMeta of catalog) {
        const downloadPath = fileMeta?.download_path;
        const blob = await downloadLocalVideoBlob(downloadPath);
        const rowIndex = Number(fileMeta?.row_index);
        await uploadSingleVideoBlobViaMultipart({
          missionId,
          droneId,
          blob,
          filename: fileMeta?.name ?? null,
          contentType: fileMeta?.content_type || 'video/mp4',
          rowIndex: Number.isFinite(rowIndex) && rowIndex > 0 ? rowIndex : null,
          rowsCount,
  ...
  const handleDronePositionChange = useCallback((droneId, nextPosition) => {
    ...
    setDrones((prev) =>
      prev.map((d) =>
        d.id === droneId
          ? { ...d, position: { lat, lng }, readyForMissionStart: false }
          : d
      )
    );
  }, []);
  ...
  const undoLastPoint = (droneId) => {
    ...
    setDrones(prev =>
      prev.map(d =>
        d.id === droneId ? { ...d, path: d.path.slice(0, -1), readyForMissionStart: false } : d
      )
    );
    ...
    return entries
      .map(([missionId, result]) => {
        ...
        return {
          ...
          rowsCount: Number(result?.rows_count ?? 0),
      ...
      for (const missionId of missionIds) {
        ...
        try {
          ...
          if (statusPayload && !cancelled) {
            const droneId = missionDroneByMissionIdRef.current.get(missionId);
            const isUploadInProgress = droneId != null && videoUploadInProgressRef.current.get(droneId);
            const prevEntry = aiPendingByMissionIdRef.current[missionKey];
            const prevObj = prevEntry === true ? {} : (prevEntry && typeof prevEntry === 'object' ? prevEntry : {});
            if (shouldDropAbandonedCompletedPending(prevObj, statusPayload, isUploadInProgress)) {
              dropStalePendingMission(missionId);
              continue;
            }
            setAiPendingByMissionId((prev) => {
              const prevEntryLocal = prev[missionKey];
              const prevObjLocal = prevEntryLocal === true ? {} : (prevEntryLocal && typeof prevEntryLocal === 'object' ? prevEntryLocal : {});
              if (!(missionKey in prev) && !isMissionTracked(missionId)) {
                return prev;
              }
              const nextEntry = buildAiPendingEntryFromStatus(statusPayload, {
                missionStatus: prevObjLocal.missionStatus ?? null,
              });
              const keepMissionPhase =
                prevObjLocal.phase === 'mission' && statusPayload?.available === false;
              return {
                ...prev,
                [missionKey]: {
                  ...prevObjLocal,
                  ...nextEntry,
                  missionStatus: nextEntry.missionStatus ?? prevObjLocal.missionStatus ?? null,
                  rowsCount: nextEntry.rowsCount ?? prevObjLocal.rowsCount ?? null,
                  phase: keepMissionPhase ? 'mission' : nextEntry.phase,
                  statusMessage: nextEntry.statusMessage ?? prevObjLocal.statusMessage ?? null,
                  updatedAt: Date.now(),
                },
              };
            });
  ...
  }, [authReady, dropStalePendingMission]);
  ...
  const startDroneFlight = useCallback(async (droneId) => {
    ...
    const firstWaypointCoords = getFirstWaypointCoords(drone);
    ...
    setDrones(prev =>
      prev.map(d => {
        ...
        return {
          ...d,
          position: firstWaypointCoords ?? d.position,
          readyForMissionStart: false,
  ...
  const completeDroneFlight = (droneId) => {
    ...
    setTimeout(async () => {
      ...
      if (missionIdForUpload) {
        try {
          if (VIDEO_FROM_FOLDER) {
            await autoUploadLocalVideosForMission(droneId, missionIdForUpload);
          } else {
            await autoUploadLocalVideosViaMultipartForMission(droneId, missionIdForUpload);
          }
        } catch (e) {
          console.warn('Local video upload failed:', e?.message ?? e);
        }
      }
      videoRowSplitStateRef.current.delete(droneId);
    }, 3000);
