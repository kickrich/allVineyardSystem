...
/** Плавное следование маркера за координатами полёта (интервал телеметрии ~100 ms). */
const DRONE_FLIGHT_MOVE_DURATION_MS = 100;
...
const OSM_BUILDINGS_API_URL = '/api/v1/osm/buildings';
const BUILDINGS_REFRESH_DEBOUNCE_MS = 200;
const OSM_DEGRADED_NOTICE =
  'Здания OSM временно недоступны — проверка маршрута через здания отключена.';
...
/** Пересекает ли отрезок маршрута контур здания или проходит через его footprint. */
...
/** Точка (lat, lng) внутри замкнутого кольца [[lat, lng], ...]. */
function pointInRing(lat, lng, ring) {
  if (!Number.isFinite(lat) || !Number.isFinite(lng) || !Array.isArray(ring) || ring.length < 3) {
    return false;
  }
  let inside = false;
...
function routeSegmentCrossesBuilding(prevPoint, clickPoint, building) {
  if (!prevPoint || !clickPoint || !building?.ring || !building?.bbox) return false;
  ...
  if (
    maxLat < building.bbox.south ||
    minLat > building.bbox.north ||
    maxLng < building.bbox.west ||
    minLng > building.bbox.east
  ) {
    return false;
  }
  const { ring } = building;
  if (routeSegmentCrossesRing(prevPoint, clickPoint, ring)) return true;
  if (pointInRing(prevPoint.lat, prevPoint.lng, ring)) return true;
  if (pointInRing(clickPoint.lat, clickPoint.lng, ring)) return true;
  const midLat = (prevPoint.lat + clickPoint.lat) / 2;
  const midLng = (prevPoint.lng + clickPoint.lng) / 2;
  if (pointInRing(midLat, midLng, ring)) return true;
  return false;
}
...
function routeSegmentCrossesBuildings(prevPoint, clickPoint, buildingList) {
  ...
  for (const b of buildingList) {
    if (routeSegmentCrossesBuilding(prevPoint, clickPoint, b)) return true;
  }
  return false;
}

function pathHasSegmentThroughBuildings(path, buildingList) {
  if (!Array.isArray(path) || path.length === 0 || !Array.isArray(buildingList) || buildingList.length === 0) {
    return false;
  }
  for (const p of path) {
    ...
    if (![lat, lng].every(Number.isFinite)) continue;
    if (buildingList.some((b) => pointInRing(lat, lng, b?.ring))) return true;
  }
...
async function fetchBuildingsFromApi(bbox, signal) {
  ...
  try {
    const res = await fetch(`${OSM_BUILDINGS_API_URL}?${params}`, {
      method: 'GET',
      signal,
      cache: 'no-store',
    });
    ...
    if (!res.ok) {
      const apiError = Array.isArray(json?.errors) ? json.errors[0] : null;
      throw new Error(apiError || `OSM buildings HTTP ${res.status}`);
    }
    const buildings = Array.isArray(json?.buildings) ? json.buildings : json?.data?.buildings;
...
export function YandexMap({
  ...
  const buildingsDegradedNoticeDismissedRef = useRef(false);
  ...
  const droneFlightMoveRafRef = useRef({});
  const droneFlightMoveActiveRef = useRef(new Set());
  ...
  useEffect(() => {
    ...
    buildingsDebounceRef.current = setTimeout(() => {
      ...
      fetchBuildingsFromApi(bbox, ac.signal)
        .then((polys) => {
          const list = Array.isArray(polys) ? polys : [];
          setBuildings(list);
          setBuildingsStatus('ready');
          if (!list.length) {
            if (!buildingsDegradedNoticeDismissedRef.current) {
              setBuildingsNotice(OSM_DEGRADED_NOTICE);
            }
          } else {
            buildingsDegradedNoticeDismissedRef.current = false;
            setBuildingsNotice(null);
          }
        })
        .catch((e) => {
  ...
  const cancelDroneFlightMoveAnim = (droneId) => {
    const id = String(droneId);
    const h = droneFlightMoveRafRef.current[id];
    if (h != null) cancelAnimationFrame(h);
    delete droneFlightMoveRafRef.current[id];
    droneFlightMoveActiveRef.current.delete(id);
  };
  ...
  const cancelDroneMarkerAnim = (droneId) => {
    ...
    cancelDroneFlightMoveAnim(droneId);
    droneRemoveAnimActiveRef.current.delete(id);
    dronePlaceTargetKeyRef.current.delete(id);
  };

  const runDroneFlightMoveAnim = (droneId, placemark, targetPos, durationMs = DRONE_FLIGHT_MOVE_DURATION_MS) => {
    ...
    try {
      ...
      try {
        placemark.geometry.setCoordinates(targetPos);
      } catch {
        /* ignore */
      }
      droneMarkerPositionKeyRef.current.set(idStr, `${targetPos[0]},${targetPos[1]}`);
      return;
    }
    ...
    if (fromKey === targetKey) {
      droneMarkerPositionKeyRef.current.set(idStr, targetKey);
      return;
    }

    droneFlightMoveActiveRef.current.add(idStr);
    ...
    const step = (now) => {
      ...
      const lat = from[0] + (targetPos[0] - from[0]) * k;
      const lng = from[1] + (targetPos[1] - from[1]) * k;
      try {
      ...
      if (u < 1) {
        droneFlightMoveRafRef.current[idStr] = requestAnimationFrame(step);
      } else {
        delete droneFlightMoveRafRef.current[idStr];
        droneFlightMoveActiveRef.current.delete(idStr);
        droneMarkerPositionKeyRef.current.set(idStr, targetKey);
      }
    };
    droneFlightMoveRafRef.current[idStr] = requestAnimationFrame(step);
      ...
      if (drone.isVisible && drone.position) {
        ...
        if (existingMarker) {
          if (routeEditMode || placementMode) {
            ...
            if (lastKey !== posKey) {
              runDroneFlightMoveAnim(drone.id, existingMarker, pos);
            }
            try {
              existingMarker.options.set('draggable', false);
            } catch {
              /* ignore */
            }
          } else if (droneFlightMoveActiveRef.current.has(idStr)) {
            cancelDroneFlightMoveAnim(drone.id);
            existingMarker.geometry.setCoordinates(pos);
            droneMarkerPositionKeyRef.current.set(idStr, posKey);
          } else {
            const lastKey = droneMarkerPositionKeyRef.current.get(idStr);
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      ...
      if (routeEditMode) {
        ...
        if (activeBoundary) {
          if (buildingsStatusRef.current === 'loading') {
            showBuildingsNotice('Загрузка зданий OSM для проверки маршрута...');
            return;
          }
          if (buildingsStatusRef.current === 'error') {
            showBuildingsNotice('Не удалось загрузить здания OSM. Проверьте, что backend запущен.');
            return;
          }
        }
        ...
        if (buildingList.length > 0) {
          if (!prev) {
            const insideBuilding = buildingList.some((b) =>
              pointInRing(clickPoint.lat, clickPoint.lng, b?.ring)
            );
            if (insideBuilding) {
              showBuildingsNotice('Нельзя ставить точку маршрута на здание (OSM).');
              return;
            }
          } else if (routeSegmentCrossesBuildings(prev, clickPoint, buildingList)) {
  ...
  useEffect(() => {
    return () => {
      ...
      Object.keys(droneFlightMoveRafRef.current).forEach(id => {
        const h = droneFlightMoveRafRef.current[id];
        if (h != null) cancelAnimationFrame(h);
      });
      dronePlaceAnimRafRef.current = {};
      droneFlightMoveRafRef.current = {};
      dronePlaceAnimActiveRef.current.clear();
      droneFlightMoveActiveRef.current.clear();
  ...
  return (
    ...
    >
      ...
      {buildingsNotice && (
        <div className="pointer-events-auto absolute top-3 left-1/2 z-[210] flex w-[min(92vw,520px)] -translate-x-1/2 items-start gap-2 rounded-xl border border-amber-500/60 bg-amber-950/90 py-2 pl-3 pr-2 text-sm text-amber-100 shadow-xl backdrop-blur-sm">
          <span className="flex-1 leading-snug">{buildingsNotice}</span>
          <button
            type="button"
            aria-label="Закрыть уведомление"
            title="Закрыть"
            className="shrink-0 rounded-md px-1.5 py-0.5 text-lg leading-none text-amber-200/80 transition-colors hover:bg-amber-900/70 hover:text-amber-50"
            onClick={() => {
              if (buildingsNotice === OSM_DEGRADED_NOTICE) {
                buildingsDegradedNoticeDismissedRef.current = true;
              }
              setBuildingsNotice(null);
            }}
          >
