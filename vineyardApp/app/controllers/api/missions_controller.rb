class Api::MissionsController < ApplicationController
  ...
  def resolve_video_for_shard_upload(mission_id:, video_id_param:, callback_token:, row_index:, rows_count:, name:, external_service_url:)
    ...
    if existing
      ...
      return { error: err, status: :unauthorized } if err
      sync_rows_count!(existing, rows_count)
  ...
  def sync_rows_count!(video, rows_count)
    return unless rows_count.present? && rows_count.positive?
    return if video.rows_count.present? && video.rows_count >= rows_count

    video.update!(rows_count: rows_count)
  end
end
