class SendVideoToVineyardAppJob < ApplicationJob
  ...
  def perform(media_upload_id)
    ...
    if video_id
      media_upload.update!(status: 'sent_to_vineyard')
      Rails.logger.info("[SendVideoToVineyardAppJob] Completed, video_id=#{video_id}")
    else
      media_upload.update!(status: 'failed')
      Rails.logger.error("[SendVideoToVineyardAppJob] Failed for media_upload_id=#{media_upload_id}")
    end
  rescue => e
