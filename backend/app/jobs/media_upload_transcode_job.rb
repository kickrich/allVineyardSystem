...
class MediaUploadTranscodeJob < ApplicationJob
  ...
  def perform(media_upload_id)
    ...
    # Уже mp4 (загрузка из папки) — не перекодируем: libx264 раздувает файл и тянет время CV.
    content_type = meta["content_type"].to_s.downcase
    if source_key.match?(/\.mp4\z/i) || content_type == "video/mp4"
