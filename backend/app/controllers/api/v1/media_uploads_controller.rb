  ...
  module V1
    class MediaUploadsController < BaseController
      ...
      def multipart_init
        ...
        if byte_size <= S3MultipartUploadService::MAX_SINGLE_PUT_BYTES
          signed = multipart_service.presign_put_object(key: key, content_type: content_type)
      ...
      def multipart_complete
        ...
        if meta["upload_mode"] == "simple_put"
          unless multipart_service.object_exists?(key: meta["key"].to_s)
            render_errors("Файл не найден в MinIO после PUT", status: :unprocessable_entity)
            return
          end
        else
          if parts.empty?
        ...
        finalize_s3_media_upload!(media_upload, meta)
        render_data(media_upload_payload(media_upload.reload), status: :created)
      rescue S3MultipartUploadService::ConfigError => e
        render_errors(e.message, status: :unprocessable_entity)
      rescue Aws::S3::Errors::ServiceError => e
      ...
      def multipart_abort
        ...
        meta = media_upload.upload_meta || {}
        unless meta["upload_mode"] == "simple_put"
      ...
      def finalize_s3_media_upload!(media_upload, meta)
        key = meta["key"].to_s
        source_url = multipart_service.object_public_url(key)
